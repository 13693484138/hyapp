'use strict';
angular.module('app').controller('mecoCtrl',['$rootScope','$state','$scope','$cookieStore','$http',function($rootScope,$state,$scope,$cookieStore,$http){

       var userid=$cookieStore.get('userid')||$rootScope.Phoneuserid;
       ...
       $http({
         ...
         headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          ...
          $scope.user_base_name=$scope.user_base_name.length>0?$scope.user_base_name:'还没有昵称';
          $scope.user_sex=data.data.RET.Sys_GX_User.USER_SEX;//性别
          $scope.user_sex=$scope.user_sex.length>0?$scope.user_sex:'请选择性别';
